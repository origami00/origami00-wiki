...
- **日期**：2026-04-28
- **决策**：将 `CalendarCard` 组件集成到 `ClockCard` 中，通过 `showCalendar` 状态在时钟视图和日历视图之间切换，移除独立的 `CalendarCard` 组件引用
- **备选方案**：
  - 保持两个独立卡片 — 首页卡片数量较多，布局冗余
  - 将时钟集成到日历中 — 时钟是更常用的查看项，应保持默认可见
  - 使用路由切换（如 /calendar）— 过于重量级，破坏首页卡片式交互模式
- **理由**：日期和日历功能高度关联，集成后减少首页卡片数量，提升布局紧凑度。点击日期进入日历的交互符合用户直觉
- **影响**：
  - `ClockCard.tsx` 新增 `useState` 管理视图切换，复用 `useCalendar` hook
  - `HomePage.tsx` 移除 `CalendarCard` 引用，卡片数从 7 个减为 6 个
  - `CalendarCard.tsx` 保留但不再被使用（可后续清理删除）
  - 响应式布局无需调整，单卡片自适应
